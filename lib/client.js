...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ClawbotSection({ useScope, scope }) {
			...
			// 当外部更新（如 host 重启 / 别的入口修改）时同步草稿
			useEffect(() => {
				setDraft((prev) => {
					if (prev === null) return { ...value };
					// 仅当外部值变化时刷新（避免覆盖正在输入的草稿）
					return prev;
				});
			...
			const save = useCallback(async () => {
				...
				setSaving(true);
				try {
					// 逐字段写入（settingsScope 支持部分字段更新）
					const entries = Object.entries(draft);
					for (const [k, v] of entries) {
						if (k in value && value[k] === v) continue; // 未修改的跳过
						await scope.set(k, typeof v === "number" ? v : String(v));
					}
					setFlash("已保存 ✓");
