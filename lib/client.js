...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ClawbotSection({ useScope, scope }) {
			...
			// 记录用户改动过的字段（key → 用户输入值），保存时按此提交，保留原类型
			const dirtyRef = useRef(new Map());
			const [saving, setSaving] = useState(false);
			const [flash, setFlash] = useState("");

			// 初始草稿来自 scope 快照；后续外部更新保持草稿
			useEffect(() => {
				setDraft((prev) => (prev === null ? { ...value } : prev));
				// eslint-disable-next-line react-hooks/exhaustive-deps
			}, []);

			const markDirty = useCallback((key, typedValue) => {
				setDraft((d) => ({ ...d, [key]: typedValue }));
				dirtyRef.current.set(key, typedValue);
			}, []);
			...
			const save = useCallback(async () => {
				...
				setSaving(true);
				try {
					// 只提交用户改动过的字段；类型按 schema 推断
					const dirty = [...dirtyRef.current.entries()];
			...
			if (!draft) return jsx("div", { className: "dcn-wrap", children: "加载中…" });
