...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const FIELDS = [
			...
			{ key: "incomingEnabled", label: "双向控制", desc: "允许手机微信发消息驱动本机 agent（需已扫码登录）", type: "switch" },
			{ key: "allowedSenders", label: "允许的微信用户", desc: "允许驱动的微信用户 ID（逗号分隔；空 = 允许所有发送者）", type: "text" },
			{ key: "incomingCwd", label: "微信任务工作目录", desc: "微信消息驱动 agent 的工作目录（空 = 默认工作区）", type: "text" },
			{ key: "runSelfTestOnStart", label: "启动自检", desc: "启动时验证 /choose 回答链路（mux+respond 闭环），结果发微信", type: "switch" },
		];
