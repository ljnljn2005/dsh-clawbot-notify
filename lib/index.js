...
/** 依赖 tools 服务（用于注册 clawbot_login 等工具）；事件监听不依赖注入。 */
const inject = ["tools"];
...
function apply(ctx, config) {
	...
	ctx.on("agent/status", ({ agent, status }) => {
		...
		if (config.notifyComplete && reason?.kind === "completed") {
			...
			void sendNotify(ctx, config, buildComplete(config, agent, excerpt));
			...
			void sendNotify(ctx, config, buildQuestion(config, agent.id, []));
		}
	});
	...
	ctx.on("agent/error", ({ agent, turn, step, error }) => {
		...
		void sendNotify(ctx, config, buildError(config, agent, turn, message));
	});
	...
	ctx.on("session/event", (session, event) => {
		...
		if (sentQuestions.has(key)) return;
		sentQuestions.add(key);
		void sendNotify(ctx, config, buildQuestion(config, session.id, parseQuestions(data.arguments)));
	});

	// ---- 登录工具注册（可选，仅当有 tools 上下文时） ----
	if (ctx.tools?.register) {
		ctx.tools.register(loginTool(ctx, config));
		ctx.tools.register(loginConfirmTool(ctx, config));
		ctx.tools.register(accountTool(ctx, config));
	}
}
